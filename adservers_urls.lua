--[[    lua script : adservers_urls.lua
	purpose    : to check the url is matched in blacklisted advertising servers or not
...
function match(args)
	...
	-- open files
  	-- local url_file = io.open("/etc/suricata/rules/adservers.url", "r")
  	local url_file = io.open("/var/lib/suricata/rules/adservers.url", "r")
	local access_file = io.open("/var/log/suricata/adservers_urls.log", "a")
